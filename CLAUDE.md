...
DB migrations live in `supabase/migrations/`, named `NNN_*.sql` — currently through 086. Key recent milestones: 063 cadence_loop, 066 platform_connections, 067 scheduled_posts, 068 metrics_sync, 069 r2_storage, 073 social_drafts_variants, 074 gsc_contentful_providers, 075 seo_posts, 076 outbound_template_type + outreach_campaigns, 077 global_outbound_templates + prospect_follow_up, 078 conversation_analyses, 079 prospect_notes, 080 prospect_enrichment_fields, 081 add_phone_to_prospects, 082 analytics_reports, 083 own_post_metrics_dedup, 084 analytics_import_sessions, 085 event_scraper_runs, 086 content_slots. Apply via Supabase SQL Editor (paste + run) or `supabase db push` after `supabase login --token <pat>` and `supabase link --project-ref <ref>`. The user typically runs migrations by hand in the SQL Editor, so the migration *number* (not the SQL) is what they need from you.
...
- **Content calendar** (individual persona, mig 086): `/content-calendar` — daily-briefing content queue for individual-persona accounts, gated by `surfaces: ["individual"]` in `nav-config.ts` AND a tenant-slug allowlist (`src/lib/content-calendar/tenant-config.ts`, currently just `aiseosa-space` — personal dogfood v1, not generalized yet). A "content slot" (`content_slots` table) is an AI-picked topic + a grounded research briefing (talking points, a sourced stat, a contrarian angle, reference links) — generated in batches ("Generate my next N", capped at `MAX_BATCH_SIZE`) via `src/lib/actions/content-calendar.ts`, run synchronously with `mapWithConcurrency` (no durable-runner tables — batch size is small enough not to need the checkpointed-run pattern used by the SEO/video runners). Topic selection + briefing generation is two AI calls per slot (`src/lib/ai/content-calendar.ts`): a cheap topic pick, then a synthesis call grounded in real SERP search results fetched between the two calls, so any stat always carries a real source URL. Trend-sourcing (`src/lib/scrape/trend-pull.ts`, Hacker News Algolia API + Serper) is deliberately a small parallel pipeline, NOT reusing Trend Scouts/Intel Feed/X Listening, to avoid coupling this unvalidated feature to infra Gruve/Sippy depend on. No literal calendar dates — `position` is the only ordering key, and "next" is computed lazily (`src/lib/services/content-calendar-lifecycle.ts`) as the lowest-position slot still `assigned`/`in_progress`; slots auto-retire to `skipped` past a staleness grace window (queue-depth capped at `MAX_QUEUE_DEPTH`). A dedicated cron (`/api/cron/content-calendar-email`, separate from the startup-persona `daily-email` cron for blast-radius reasons) emails the next unposted slot each morning, or a "generate more" nudge if the queue is empty. Video upload reuses only the signed-upload-URL R2 mechanism from `video-generate.ts`, not its `video_assets` table (different lifecycle — a founder's manual upload, not an AI-rendered clip). LLM eval suite deliberately deferred — the founder's own usage is the eval for now (see `TODOS.md` under `## P9`).
