...
DB migrations live in `supabase/migrations/`, named `NNN_*.sql` — currently through 092. Key recent milestones: 063 cadence_loop, 066 platform_connections, 067 scheduled_posts, 068 metrics_sync, 069 r2_storage, 073 social_drafts_variants, 074 gsc_contentful_providers, 075 seo_posts, 076 outbound_template_type + outreach_campaigns, 077 global_outbound_templates + prospect_follow_up, 078 conversation_analyses, 079 prospect_notes, 080 prospect_enrichment_fields, 081 add_phone_to_prospects, 082 analytics_reports, 083 own_post_metrics_dedup, 084 analytics_import_sessions, 085 event_scraper_runs, 086 content_slots, 087 content_slots_scheduled_date, 088 api_v1_token_scopes, 089 scheduled_posts_reconcile, 090 own_post_metrics_full_unique_index, 091 approval_requests, 092 oauth_authorization_server. Apply via Supabase SQL Editor (paste + run) or `supabase db push` after `supabase login --token <pat>` and `supabase link --project-ref <ref>`. The user typically runs migrations by hand in the SQL Editor, so the migration *number* (not the SQL) is what they need from you.
...
**`create table if not exists` silently no-ops** if a narrower-shaped table already exists from an earlier, divergent migration — later `alter table add column` migrations still succeed individually, masking the gap until something queries a column that was never actually added (089 reconciles a `scheduled_posts` case exactly like this — 10 columns from migration 067 were missing in production). If a table's shape looks wrong, check for this before assuming the migration never ran.

**`ON CONFLICT` can't target a partial unique index** (`create unique index ... where col is not null`) unless the same `WHERE` is repeated in the `ON CONFLICT` clause — and Supabase JS's `.upsert(data, {onConflict: "col1,col2"})` has no way to express that. Symptom: "there is no unique or exclusion constraint matching the ON CONFLICT specification" on every upsert. Fix is a full (non-partial) unique index, not a client-side workaround (090).
...
├── oauth/authorize/      ← browser-facing OAuth 2.1 consent page for MCP clients (see below)
├── api/                  ← route handlers
│   ├── cron/             (generate-briefs, scrape-trends, weekly-digest — bearer-gated)
│   ├── ext/              (Chrome extension endpoints — tenant-API-token auth, CORS)
│   ├── v1/               (versioned, scoped, token-auth REST API for external AI operators — see below)
│   ├── oauth/            (RFC 8414/7591/9728 endpoints backing the MCP OAuth 2.1 flow)
│   ├── [transport]/      (the actual MCP server route — resolves to the public /api/mcp URL, see below)
...
**Prompt files + Vercel bundling gotcha.** Most AI calls build prompts inline in TS (e.g. `generate-seo-blog.ts`'s `buildSystemPrompt`/`buildUserPrompt`), but a few (blog expand/refine/apply-feedback, the scoring rubrics) load Markdown+YAML-frontmatter prompt files from `prompts/<area>/<name>.md` via `loadPrompt()` in `src/lib/ai/prompts.ts`. Next's file tracer only follows imports, not arbitrary `fs.readFileSync` calls, so `prompts/` must be force-included per server function via `outputFileTracingIncludes` in `next.config.ts` — omitting it means the prompt files are missing in the deployed bundle and every affected AI call 404s/throws in production while working fine locally.
...
- **Ads Platform** (real Meta + TikTok ads, migrations 094-097): replaces the old manual-entry `campaigns` table (kept only for tenants that have never connected a real account — `computeAdSpendStat` in `dashboard.ts` prefers `ad_accounts`/`ad_insights_daily` the moment one exists). Meta connects via Composio's `metaads` toolkit (same `connected_accounts` OAuth pattern as Instagram/TikTok/LinkedIn — verified tool slugs in `src/lib/composio/ads-executors.ts`); TikTok has no Composio toolkit so it's direct OAuth (`src/lib/integrations/tiktok-ads.ts`, `tiktok_ads_connections` table, `/api/integrations/tiktok-ads/{connect,callback}`). `ad_accounts` is the unified join point every downstream table (`ad_campaigns`/`ad_sets`/`ads`/`ad_creatives`/`ad_insights_daily`) keys off, regardless of platform. Two sync crons: `sync-ad-structure` (daily) and `sync-ad-insights` (hourly, rolling 3-day window to catch attribution revisions). **Blended ROAS** (`src/lib/attribution/ads.ts`) joins ad spend against Pulse's own `orders.utm_campaign` — the actual differentiator over the platform's self-reported ROAS, matched via `ad_campaigns.utm_campaign_override` (defaults to a slugified guess from the campaign name, confirmed/corrected per-campaign, never presented as fact until confirmed — see `matchConfidence`). Server-side conversions: Meta CAPI (`src/lib/integrations/meta-capi.ts`, needs a manually-entered pixel ID + Events-Manager-generated token per ad account — distinct credential from the OAuth connection) and TikTok Events API, both fired best-effort from `/api/orders/webhook` on `paid`/`fulfilled` status via `fireOrderConversionEvents`. Competitor ad intel (`sync-competitor-ads` cron) uses Meta's public Ad Library API — real for ordinary commercial competitors only as creative/longevity/platform data, spend/impression *numbers* are gated to political/EU-regulated ads only, don't build UI implying otherwise. TikTok has no equivalent open API (Commercial Content Library is EU-only, research-gated) — deliberately not built. Creative fatigue: Meta pushes its own `creative_fatigue` webhook (`/api/webhooks/meta-ads`, HMAC-verified) with zero ML needed; TikTok has no native signal so `detect-creative-fatigue` derives it from a 3-day CTR-decline + frequency-threshold trend. Budget guardrail rules (`ad_budget_rules` + `evaluate-ad-budget-rules` cron, every 30min) require a metric to hold past threshold for every day in `hold_days` (not just an average) before acting — the anti-noise pattern this whole space uses. MCP/REST: `src/lib/mcp/tools/ads-platform.ts` + `/api/v1/ads/*`.
...
`/content-calendar` — a topic + research-briefing engine for solo creators who know they want to post but don't know what to say. Gated by `surfaces: ["individual"]` in `nav-config.ts`. Originally also allowlisted to a single dogfood tenant via `isContentCalendarEnabledForTenant()` (`src/lib/content-calendar/tenant-config.ts`) — that allowlist has since been lifted (the function now returns `true` for every tenant), but the check stays wired at every call site (page, `/api/v1/content-calendar`, cron, MCP tool) as a single choke point for a future re-gating. Deliberately does NOT reuse Trend Scouts/Intel Feed/X Listening for trend-sourcing — a small parallel pipeline instead, to avoid coupling this feature to infra Gruve/Sippy depend on.
...
- **`SlotPanel` (`client.tsx`) has two separate title-editing affordances — the inline title editor and the full-brief edit form — that both read/write the same `titleDraft` state.** Toggling into or out of either mode without resyncing every draft field from the canonical `slot` prop lets an abandoned, uncommitted edit in one form silently ride along into a save triggered by the other (a real data-loss bug, fixed via `openEditBrief`/`closeEditBrief` helpers that resync on every entry/exit). Any new edit-mode toggle added to this panel must follow the same resync-on-entry/exit pattern rather than assuming a `useEffect` keyed on `slot.*` fields is enough — that effect only fires after an actual save, not on every mode switch.

## `/api/v1` REST API + MCP server (external AI operators)

Full reference (all endpoints, all 53 MCP tools, deviations, auth flow) lives in `docs/API-V1.md` — treat it as the source of truth over this summary, and `GET /api/v1/manifest` as the source of truth over the doc (self-generated from `src/lib/api/manifest.ts`, can't drift from the real routes).
...
- **MCP server**: every `/api/v1` capability re-exposed as an MCP tool at the public URL `/api/mcp` (route file is `src/app/api/[transport]/route.ts` — `[transport]` is a literal segment `mcp-handler` resolves itself, not a real `mcp/` folder; nesting one would double the path). Built on `mcp-handler` + `@modelcontextprotocol/sdk`. Same auth, same service layer, same tenant isolation as the REST API — a second transport onto the identical gate, not a parallel implementation. Streamable HTTP only for now (SSE needs Redis for session state, not yet provisioned).
- **Dual auth on the MCP route**: `withMcpAuth()`'s `verifyToken()` accepts either a `pulse_ext_...` static token (via `resolveApiToken()`, identical to the REST path) or a self-issued OAuth 2.1 access token (via `verifyAccessToken()`, `src/lib/oauth/tokens.ts`). Both produce the same `AuthInfo` shape so every tool's `requireToolScope()` (`src/lib/api/mcp-context.ts`) works unmodified either way. **No tool ever accepts a tenant argument from the model** — the token/session is the only source of tenant identity.
- **OAuth 2.1 authorization server** (MCP-route-only — `/api/v1` REST and the Chrome extension are untouched): full RFC 8414 (`/.well-known/oauth-authorization-server`) + RFC 9728 (`/.well-known/oauth-protected-resource`) + RFC 7591 Dynamic Client Registration (`POST /api/oauth/register`) so a client like Cowork can connect with just a URL, no manual token minting. Browser consent page at `/oauth/authorize` goes through the normal Supabase Auth login gate, then lets the user pick which tenant (scoped to memberships where their role is `owner`/`admin`) to authorize. PKCE mandatory, S256 only. Access tokens are short-lived (1hr) self-contained HS256 JWTs signed with `MCP_OAUTH_JWT_SECRET`; refresh tokens are opaque, DB-stored, hashed, and rotate on every use. `POST /api/oauth/revoke` is not implemented — revoke early by deleting the `oauth_refresh_tokens` row directly.
- **Adding a new capability**: add the service call to both the REST route (`src/app/api/v1/<area>/`) and the matching MCP tool (`src/lib/mcp/tools/<area>.ts`, registered in `src/app/api/[transport]/route.ts`) — they're meant to stay twins, wrapping the same service function, not diverge.
...
- **Toasts** — `toast.success/error/info(title, subtitle?, action?)` from `src/components/ui/Toaster.tsx`. The `<Toaster />` is mounted once in `(app)/layout.tsx`. The optional third `action: {label, onClick}` param renders an inline text-button in the toast (e.g. "Undo") — use this instead of building a bespoke undo affordance when a destructive/silent action (drag-reschedule, bulk status change) needs a one-click reversal.
