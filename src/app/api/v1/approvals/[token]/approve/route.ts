// Approve a pending approval request. Auth is the signed token in the
// path itself (verifyApprovalToken) — there is no session and no bearer
// tenant_api_token here; the public /approve/[token] page is the only
// intended caller. Never accepts a tenant argument from the client for
// the same reason every other /api/v1 route doesn't: the token is the
// only source of truth for which row this request may act on.
