// Daily recommendation generation (PULSE-SEO-SPEC.md §9). Bearer-gated;
// one cron_runs row per invocation.

import { NextResponse } from "next/server";
import { verifyFromRequest } from "@/lib/cron/auth";
import { withCronRun } from "@/lib/cron/run-tracker";
import { generateRecommendations } from "@/lib/seo/rec-generator";
...
export async function POST(req: Request) {
  ...
  const result = await withCronRun("seo-rec-generate", () =>
