...
- [ ] **`POST /api/oauth/revoke` (RFC 7009) for the MCP OAuth server** — the OAuth 2.1 authorization server added for the Cowork "URL only" connector flow (`docs/API-V1.md` → "OAuth 2.1 for MCP clients") has no revoke endpoint. Today the only way to kill a refresh token early (lost device, compromised client) is deleting its `oauth_refresh_tokens` row directly in the DB. Not required for Cowork to connect, deliberately deferred out of that build. (~1-2 hrs — mirror the existing `consumeAuthorizationCode`/`rotateRefreshToken` conditional-UPDATE pattern in `src/lib/oauth/`)
- [ ] **`whatsapp_accounts.access_token` stored in plaintext** — unlike `platform_connections` (AES-encrypted via `PLATFORM_TOKEN_KEY`), `saveWhatsappAccount` (`src/lib/actions/whatsapp.ts`) writes the raw Meta access token straight into the DB column with no encryption. Pre-existing (the action was built but had no UI calling it until 2026-08-07, when a WhatsApp connect form was finally wired up in `/broadcasts` — see `WhatsAppConnectCard.tsx`). Should follow the same AES pattern as platform_connections: encrypt on write, decrypt only where the token is actually used to send (`src/lib/integrations/whatsapp.ts`).
